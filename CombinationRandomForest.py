...
def generateData():
  ...
  for item in subsetIndex:
    featureSubSet.append(features[item])
  print(featureSubSet)
  print("*"*50)
  # for i
...
def generateForest(split, sizeOfForest):
  ...
  for i in range(sizeOfForest):
    ...
    for i in range(int(len(trainingSet)/2)):
      randomTrainingSet.append(trainingSet[int(random.uniform(0,len(trainingSet)))]) #random generate sub training set with replacement
    ...
    ID3= makeDecisionTree(randomTrainingSet,featureSubSet)
